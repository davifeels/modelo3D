...
@dataclass
class JointParams:
    pin_diameter: float = 3.0    # mm — diâmetro do pino
    pin_depth: float = 8.0       # mm — profundidade do pino
    tolerance: float = 0.2       # mm — folga no furo
    n_pins: int = 1              # quantidade de pinos na seção


def _cylinder_transform(direction: np.ndarray, center: np.ndarray) -> np.ndarray:
    """4x4 que alinha o eixo Z do cilindro com `direction` e move o centro para `center`."""
...
def _make_pin(radius: float, depth: float, origin: np.ndarray, normal: np.ndarray) -> trimesh.Trimesh:
    """Cria cilindro (pino) com base em `origin`, estendendo na direção de `normal`."""
...
def _make_hole(radius: float, depth: float, origin: np.ndarray, normal: np.ndarray) -> trimesh.Trimesh:
    """Cria cilindro (furo) começando em `origin`, indo na direção -normal."""
    hole = trimesh.creation.cylinder(radius=radius, height=depth + 1.0, sections=32)
    # furo vai em direção oposta ao pino (entra na parte B)
    center = origin - normal * (depth / 2.0)
...
def _offsets_for_n_pins(n: int, cut_section_pts: np.ndarray, normal: np.ndarray) -> list:
    """Distribui n pinos ao longo da seção transversal."""
    if n == 1 or len(cut_section_pts) == 0:
        return [cut_section_pts.mean(axis=0) if len(cut_section_pts) > 0 else np.zeros(3)]

    # projeta pontos no plano perpendicular ao normal e usa extremos
    centroid = cut_section_pts.mean(axis=0)
    # vetores no plano do corte
    perp = np.array([-normal[1], normal[2], normal[0]])  # qualquer perpendicular
    ...
    for i in range(n):
        t = (i + 1) / (n + 1)
        offsets.append(centroid + perp * (mn + t * span - projections.mean()))
    return offsets
...
def add_joints(
    ...
    normal aponta de part_b → part_a (direção positiva do corte).
    Retorna (part_a_com_pinos, part_b_com_furos).
    """
    ...
    for origin in origins:
        ...
        try:
            ...
            pass  # pino não adicionado — mesh problemática

        try:
            result_b = trimesh.boolean.difference(result_b, hole, engine="manifold")
        except Exception:
            pass  # furo não feito — mesh problemática
